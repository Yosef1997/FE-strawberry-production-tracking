    ...
    <>
      ...
      <Table>
        <TableCaption className='text-start'>
          A list of yield, reject due to pest, and reject due to diseases based
          on weather data such as humidity, rain fall, and temperature each week
        </TableCaption>
        <TableHeader>
          <TableRow>
            <TableHead>Week Number</TableHead>
            <TableHead>Humidity {`(%)`}</TableHead>
            <TableHead>Rain Fall {`(mm)`}</TableHead>
            <TableHead>Temperature {`(C)`}</TableHead>
            <TableHead>Yield {`(kg)`}</TableHead>
            <TableHead>Reject Due To Pest {`(kg)`}</TableHead>
            <TableHead>Reject Due To Diseases {`(kg)`}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {data?.content.length === 0 ? (
            ...
            data?.content.map((content) => (
              ...
              >
                ...
                <TableCell className='font-medium text-center'>
                  {content.totalRejectDueToPest}
                </TableCell>
                <TableCell className='font-medium text-center'>
                  {content.totalRejectDueToDisease}
                </TableCell>
              </TableRow>
            ))
          )}
